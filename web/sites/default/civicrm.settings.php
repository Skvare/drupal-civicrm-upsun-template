...
if (PHP_SAPI == 'cli') {
  // Load Composer's autoloader to ensure all dependencies are available.
  // Required to enable CiviCRM functionality in CLI mode,
  // including support for the Platform.sh configuration library.
  if (isset($GLOBALS['_composer_autoload_path']) && file_exists($GLOBALS['_composer_autoload_path'])) {
    require_once $GLOBALS['_composer_autoload_path'];
  }
  else {
    $vendorAutoloadFile = dirname(__DIR__, 3) . '/vendor/autoload.php';
...
if ($platformsh->inRuntime()) {
  ...
  if ($platformsh->hasRelationship('database')) {
    ...
    $drupal_database_user = $creds['username'];
    $drupal_database_password = $creds['password'];
    $drupal_database_name = $creds['path'];
    $drupal_database_host = $creds['host'];
    $drupal_database_port = $creds['port'];
...
// Not in platform runtime. Set local variables.
  ...
  $site_uri = rtrim(getenv('SITE_URI'), "/") . "/";
  $drupal_database_user = getenv('DATABASE_USER');
  $drupal_database_password = getenv('DATABASE_PASSWORD');
  $drupal_database_name = getenv('DRUPAL_DATABASE');
  $drupal_database_host = getenv('DATABASE_HOST');
  $drupal_database_port = getenv('DATABASE_PORT') ?? 3306;
  ...
  $civicrm_database_host = getenv('DATABASE_HOST');
  $civicrm_database_port = getenv('DATABASE_PORT') ?? 3306;;
}
...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
  define( 'CIVICRM_UF_DSN', 'mysql://' . $drupal_database_user . ':' . $drupal_database_password . '@' . $drupal_database_host . ':' . $drupal_database_port . '/' . $drupal_database_name . '?new_link=true');
}
...
if (!defined('CIVICRM_TEMPLATE_COMPILEDIR')) {
  define( 'CIVICRM_TEMPLATE_COMPILEDIR', $repo_root . '/web/sites/default/civicrm/templates_c/');
}
...
if (!defined('CIVICRM_SITE_KEY')) {
  define( 'CIVICRM_SITE_KEY', $_ENV['CIVICRM_SITE_KEY']);
}
...
if (!defined('CIVICRM_CRED_KEYS') ) {
  define( 'CIVICRM_CRED_KEYS', $_ENV['CIVICRM_CRED_KEYS']);
}
...
if (!defined('CIVICRM_SIGN_KEYS') ) {
  define( 'CIVICRM_SIGN_KEYS', $_ENV['CIVICRM_SIGN_KEYS']);
}
...
// define( 'CIVICRM_MAIL_LOG', '/var/www/web/distro9.skvare.com/web/sites/default/files/civicrm/templates_c//mail.log');
// }
...
if (!defined('CIVICRM_DB_CACHE_CLASS')) {
  $caching_type = 'ArrayCache';
  if (!empty($_ENV['CIVICRM_CACHING_TYPE'])) {
    $caching_type = $_ENV['CIVICRM_CACHING_TYPE'];
  }
  define('CIVICRM_DB_CACHE_CLASS', $caching_type);
}
...
if (!defined('CIVICRM_DB_CACHE_HOST')) {
  if (CIVICRM_DB_CACHE_CLASS === 'Redis') {
    ...
    define('CIVICRM_DB_CACHE_HOST', $redis['host']);
  }
  else {
...
if (!defined('CIVICRM_DB_CACHE_PORT')) {
  if (CIVICRM_DB_CACHE_CLASS === 'Redis') {
    $redis = $platformsh->credentials('redis');
    define('CIVICRM_DB_CACHE_PORT', $redis['port'] );
  }
  else {
...
$civicrm_setting['Directory Preferences']['uploadDir'] = $repo_root . '/web/sites/default/files/civicrm/upload/';
$civicrm_setting['Directory Preferences']['imageUploadDir'] = $repo_root . '/web/sites/default/files/civicrm/persist/contribute/';
$civicrm_setting['Directory Preferences']['customFileUploadDir'] = $repo_root . '/web/sites/default/files/civicrm/custom/';
$civicrm_setting['Directory Preferences']['customTemplateDir'] = $repo_root . '/web/sites/default/civicrm/custom/templates/';
$civicrm_setting['Directory Preferences']['customPHPPathDir'] = $repo_root . '/web/sites/default/civicrm/custom/php/';
$civicrm_setting['Directory Preferences']['extensionsDir'] = $repo_root . '/web/sites/default/civicrm/extensions/';
$civicrm_setting['URL Preferences']['userFrameworkResourceURL'] = $site_uri . 'libraries/civicrm/core/';
...
$civicrm_paths['civicrm.files'] = array(
  'url' => $site_uri . 'sites/default/files/civicrm/',
  'path' => $repo_root . '/web/sites/default/files/civicrm/',
);
...
$civicrm_paths['civicrm.log'] = [
  'path' => $repo_root . '/web/sites/default/files/civicrm/ConfigAndLog/',
];

if (!defined('CIVICRM_CONFIGLOGDIR') ) {
